...
bookings = ''
total_booking = 0
longest_serving_years = 0

output = open('gmail.txt', 'a')

connection = open('staff.txt', 'r')
for line in connection:
    line = line.split()
    years = int(line[2])
    if years > longest_serving_years:
        longest_serving_years = int(line[2])
        longest_serving_first_name = line[0]
        longest_serving_surname_name = line[1]
print(f"Our most experienced member of staff has {longest_serving_years} years' experience.")
print(f"{longest_serving_first_name} {longest_serving_surname_name} is ready to welcome you!")
